...
# State is Visited, Current, Paintings

Cities = range(len(sales))

expected = [500*(0*sales[k][0] + 1*sales[k][1] + 2*sales[k][2]) for k in Cities]

def Artist(visited, curr, paintings):
    if len(visited) == 4 or paintings == 0:
        return (-distances[curr][0],0,paintings)
    Best = (-999999, -1)
    for nextCity in Cities[1:]:
        if nextCity not in visited:
            ExpectedCost = -distances[curr][nextCity]
            for s in range(3):
                ExpectedCost += sales[nextCity][s]*(500*min(s, paintings)+Artist(visited | set([nextCity]), nextCity, max(paintings-s,0))[0])
            Best = max(Best,(ExpectedCost,nextCity))
    return Best

# Expected 
print(Artist(set(), 0, 5))